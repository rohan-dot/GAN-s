...
class Workspace:
    ...
    def __init__(self, repo: pathlib.Path, run: pathlib.Path | None, tag: str):
...
{_paths(ws)}
Implement PLAN.md. Before each edit, read the target region with read_file so your str_replace old-text is exact. Add the tests, run the test command, fix until green, add the CHANGELOG.md entry, commit. Save IMPLEMENT.md with the actual outputs, then done.
"""
...
{_paths(ws)}
Review the change: `git diff {base} --stat`, then `git diff {base} -- <file>` per changed file (page long ones with `| sed -n '1,200p'`), and run the test command yourself.
...
def write_campaign_md(main: Workspace, results: list, base: str) -> None:
    lines = [f"# CAMPAIGN — {now()}", "", f"Base: {base}",
             f"Branch: {git(main.repo, 'rev-parse', '--abbrev-ref', 'HEAD').stdout.strip()}", "",
             "| # | item | area | status | branch |", "|---|---|---|---|---|"]
    for r in results:
        it = r["item"]
        lines.append(f"| {it['priority']} | {it['id']} — {it['title']} | {it['area']} | {r['status']} | {r['branch']} |")
    lines += ["", "## Not merged", ""]
    lines += [f"- {r['item']['id']}: {r['status']} — {r.get('detail','')[:500]} (worktree kept at {r['worktree']})"
...
def package(main: Workspace, base: str, handoff_name: str) -> None:
    files = [f for f in git(main.repo, "diff", "--name-only", base).stdout.split() if re.match(r"^(src|tests)/.*\.py$", f)]
    if not files:
...
def main() -> None:
    ...
    main_ws = Workspace(repo, run, tag="main")
    (main_ws.crew / "TASK.md").write_text(task)
    dirty = git(repo, "status", "--short").stdout.strip()

    log(f"[crew] repo={repo}\n[crew] run={run}\n[crew] base={base} branch={git(repo,'rev-parse','--abbrev-ref','HEAD').stdout.strip()}")
